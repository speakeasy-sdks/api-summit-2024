...
export function channelSelectToJSON(channelSelect: ChannelSelect): string {
  return JSON.stringify(ChannelSelect$outboundSchema.parse(channelSelect));
}

export function channelSelectFromJSON(
  jsonString: string,
): SafeParseResult<ChannelSelect, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => ChannelSelect$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'ChannelSelect' from JSON`,
  );
}
