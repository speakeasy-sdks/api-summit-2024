...
export function roleSelectToJSON(roleSelect: RoleSelect): string {
  return JSON.stringify(RoleSelect$outboundSchema.parse(roleSelect));
}

export function roleSelectFromJSON(
  jsonString: string,
): SafeParseResult<RoleSelect, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => RoleSelect$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'RoleSelect' from JSON`,
  );
}
