...
export function mentionableSelectToJSON(
  mentionableSelect: MentionableSelect,
): string {
  return JSON.stringify(
    MentionableSelect$outboundSchema.parse(mentionableSelect),
  );
}

export function mentionableSelectFromJSON(
  jsonString: string,
): SafeParseResult<MentionableSelect, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => MentionableSelect$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'MentionableSelect' from JSON`,
  );
}
